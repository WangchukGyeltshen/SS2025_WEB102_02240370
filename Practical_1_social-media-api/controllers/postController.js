...
const asynclHandler = require('./middleware/async');
const { posts, users } = require('./utils/mockData');

// @desc  Get all posts
// @route  GET /api/posts
// @access  Public
exports.getPosts = asynclHandler(async (req, res, next) => {
    // Pagination
...
exports.createPost = asynchandler(async (req, res, next) => {
    ...
    user_id: userId,
    created_at: new Date().toIS0String().slice(0, 10)
    };
    posts.push(newPost);
    res.status(201).json({
    success: true,
...
// @access Private (we'll simulate this)
exports.updatePost = asynclhandler(async (req, res, next) => {
    // Simulate authentication
    const userId = req.header('X-User-Id');
    if (!userId) {
    return next(new ErrorResponse('Not authorized to access this route', 401));
